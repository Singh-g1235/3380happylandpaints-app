...
export async function getProductByIdService(productId) {
  ...
  return fetch(
    `${process.env.REACT_APP_API_BASE_URL}admin/product/${productId}`
  ).then((response) => {
...
export async function getProductsService() {
  ...
  return fetch(`${process.env.REACT_APP_API_BASE_URL}cart/`).then((response) =>
    response.json()
  );
}
...
export async function getCart() {
  return fetch(
    `${process.env.REACT_APP_API_BASE_URL}cart/products`
  ).then((res) => res.json());
}
